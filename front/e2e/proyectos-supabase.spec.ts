...
const EMAIL = process.env.SMOKE_EMAIL ?? 'owner@estudiolua.com';
const PASSWORD = process.env.SMOKE_PASSWORD ?? 'demo1234Seed!';
...
test('login → dashboard (consola de tarjetas) con proyectos de Supabase', async ({ page }) => {
  ...
  await expect(page.getByText(/Estudio L[uú]a/i).first()).toBeVisible({ timeout: 15_000 });

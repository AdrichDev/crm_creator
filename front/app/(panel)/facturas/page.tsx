...
export default function Page() {
  ...
  return (
    <ModuleGuard module="facturas">
      ...
      <Table head={['Nº', 'Cliente', 'Fecha', 'Total', 'Estado', 'Docs', '']}>
        {items.map((f) => (
          <tr key={f.id}>
            <Td className="font-medium text-white">{f.numero}</Td>
            <Td>{f.cliente}</Td>
            <Td>{f.fecha}</Td>
            ...
            <Td>
              <div className="flex justify-end">
                <button className="row-action edit" title="Ver detalles" onClick={() => setInfo(f)}>
        ...
        {actual && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div><span className="text-[var(--panel-muted)]">Cliente</span><p className="text-white">{actual.cliente}</p></div>
              <div><span className="text-[var(--panel-muted)]">Fecha</span><p className="text-white">{actual.fecha}</p></div>
              <div><span className="text-[var(--panel-muted)]">Total</span><p className="text-white">€{Number(actual.total).toFixed(2)}</p></div>

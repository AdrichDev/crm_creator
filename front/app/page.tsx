      ...
      <main className="mx-auto max-w-5xl px-6 py-8">
        ...
        {projects.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-gray-300 bg-white py-20 text-center">
            <p className="font-display text-lg text-gray-800">Aún no hay proyectos</p>
            <p className="mt-1 text-sm text-gray-400">Crea el primero para un cliente y genera su paquete.</p>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {projects.map((p) => {
              ...
              return (
                <div key={p.id} className="group rounded-2xl border border-gray-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate font-display text-lg font-semibold text-gray-900">{p.config.business.name}</p>
                      <p className="text-xs text-gray-500">{v.emoji} {v.label}</p>
                    </div>
                    {p.generatedAt && <span className="rounded-full bg-green-100 px-2 py-0.5 text-[11px] font-medium text-green-700">Generado</span>}
                  </div>
                  <p className="mt-3 text-xs text-gray-400">{active.length} módulos: {active.slice(0, 4).map((m) => p.config.terminology[m.termKey] ?? m.defaultLabel).join(', ')}{active.length > 4 ? '…' : ''}</p>
                  <div className="mt-4 flex flex-wrap gap-2">
                    <button onClick={() => abrir(p.id)} className="inline-flex items-center gap-1.5 rounded-lg border border-gray-300 px-3 py-1.5 text-xs font-medium text-gray-700 transition hover:border-gray-900 hover:text-gray-900"><FolderOpen className="h-3.5 w-3.5" /> Abrir</button>
                    <button onClick={() => editar(p.id)} className="inline-flex items-center gap-1.5 rounded-lg border border-gray-300 px-3 py-1.5 text-xs font-medium text-gray-700 transition hover:border-gray-900 hover:text-gray-900"><Pencil className="h-3.5 w-3.5" /> Editar</button>
